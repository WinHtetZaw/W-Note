...
import { fetchAllMembersService } from "../services/fetch-all-members-service";

export async function fetchAllMembers(workspaceId: string) {
  const [error, data] = await fetchAllMembersService(workspaceId);
