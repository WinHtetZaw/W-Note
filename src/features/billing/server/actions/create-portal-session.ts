...
import { createPortalSessionService } from "../../services/create-portal-session-service";

export async function createPortalSession(workspaceId: string) {
  const [error, data] = await createPortalSessionService({ workspaceId });
  ...
  switch (reason) {
    ...
    case "INSUFFICIENT_PERMISSION":
      return { code: ErrorCode.Forbidden, reason };
    case "STRIPE_CUSTOMER_N0T_FOUND":
