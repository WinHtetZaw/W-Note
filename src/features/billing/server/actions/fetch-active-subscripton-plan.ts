...
import { fetchActiveSubscriptionPlanService } from "../../services/fetch-active-subscription-plan-service";

export async function fetchActiveSubscriptionPlan(workspaceId: string) {
  const [error, data] = await fetchActiveSubscriptionPlanService({
    workspaceId,
  });
